...
function updateDevicesTable() {
  fetch('/api/devices')
    ...
    .then(data => {
      ...
      const onlineDevices = data.devices.filter(device => device.status === 'Online');
      const offlineDevices = data.devices.filter(device => device.status === 'Offline');
      
      // Update the devices view HTML structure
      const devicesView = document.getElementById('devices-view');
      devicesView.innerHTML = 
        '<div class="section">' +
          '<h3>🟢 Online Devices (' + onlineDevices.length + ')</h3>' +
          '<div style="overflow-x: auto;">' +
            '<table class="table">' +
              '<thead>' +
                '<tr>' +
      ...
      if (onlineDevices.length === 0) {
        ...
        onlineDevices.forEach(device => {
          ...
          onlineHtml += '<td>' + device.batteryLevel + '%</td>';
          onlineHtml += '<td><span class="receiver-link" onclick="showReceiverDetails(\\''+device.receiverName+'\\')">📡 ' + device.receiverName + '</span></td>';
          onlineHtml += '<td>' + (device.signalStrength || 'N/A') + (device.signalStrength ? ' dBm' : '') + '</td>';
          onlineHtml += '<td>' + (device.lastSeen > 120 ? Math.floor(device.lastSeen/60) + 'm ago' : device.lastSeen + 's ago') + '</td>';
          onlineHtml += '<td style="font-size:0.8rem;">' + new Date(Date.now() - (device.lastSeen * 1000)).toLocaleString() + '</td>';
          onlineHtml += '<td>' + device.totalUpdates + '</td>';
          onlineHtml += '<td style="font-size:0.8rem;">' + device.firstSeen + '</td>';
          onlineHtml += '</tr>';
        });
      }
      document.getElementById('onlineDevicesTable').innerHTML = onlineHtml;
      ...
      if (offlineDevices.length === 0) {
        ...
        offlineDevices.forEach(device => {
          ...
          offlineHtml += '<td>' + device.batteryLevel + '%</td>';
          offlineHtml += '<td><span class="receiver-link" onclick="showReceiverDetails(\\''+device.receiverName+'\\')">📡 ' + device.receiverName + '</span></td>';
          offlineHtml += '<td>' + (device.signalStrength || 'N/A') + (device.signalStrength ? ' dBm' : '') + '</td>';
          offlineHtml += '<td><span style="color:#ef4444;">' + formatTime(device.offlineTime) + '</span></td>';
          offlineHtml += '<td style="font-size:0.8rem; color:#94a3b8;">' + new Date(Date.now() - (device.offlineTime * 1000)).toLocaleString() + '</td>';
          offlineHtml += '<td>' + device.totalUpdates + '</td>';
          offlineHtml += '<td style="font-size:0.8rem;">' + device.firstSeen + '</td>';
          offlineHtml += '</tr>';
        });
      }
      document.getElementById('offlineDevicesTable').innerHTML = offlineHtml;
    })
    .catch(() => {
      // Error fallback - show error in both tables
      const devicesView = document.getElementById('devices-view');
      devicesView.innerHTML = 
        '<div class="section">' +
          ...
          '<div style="overflow-x: auto;">' +
            '<table class="table">' +
              '<tbody>' +
                '<tr><td colspan="10" class="empty-state" style="color:#ef4444;">Failed to load devices</td></tr>' +
              '</tbody>' +
            '</table>' +
          '</div>' +
        '</div>';
