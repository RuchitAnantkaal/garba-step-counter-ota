...
void setupWebServer() {
  ...
  server.on("/api/set-forward", HTTP_POST, handleSetForward);
  server.on("/api/set-manual-rest", HTTP_POST, handleSetManualRest);
  server.on("/api/set-manual-forward", HTTP_POST, handleSetManualForward);
...
void handleRoot() {
  ...
  html += "</div>";
  html += "<div style='margin: 15px 0; padding: 15px; background: #f3e5f5; border-radius: 8px; border: 1px solid #9C27B0;'>";
  html += "<h4 style='margin: 0 0 10px 0; color: #6A1B9A;'>Manual Calibration</h4>";
  html += "<div style='margin: 10px 0;'>";
  html += "<label for='manualRestInput'>Rest Position:</label>";
  html += "<div style='display: flex; gap: 10px; align-items: center;'>";
  html += "<input type='number' id='manualRestInput' step='0.1' placeholder='Enter rest position value' style='flex: 1;'>";
  html += "<button onclick='setManualRest()' class='calibrate-btn' style='background: #4CAF50; padding: 8px 15px;'>Set Rest</button>";
  html += "</div>";
  html += "</div>";
  html += "<div style='margin: 10px 0;'>";
  html += "<label for='manualForwardInput'>Forward Position:</label>";
  html += "<div style='display: flex; gap: 10px; align-items: center;'>";
  html += "<input type='number' id='manualForwardInput' step='0.1' placeholder='Enter forward position value' style='flex: 1;'>";
  html += "<button onclick='setManualForward()' class='calibrate-btn' style='background: #2196F3; padding: 8px 15px;'>Set Forward</button>";
  html += "</div>";
  html += "</div>";
  html += "</div>";
  html += "</div>";
  ...
  html += "function setManualRest() {";
  html += "const value = parseFloat(document.getElementById('manualRestInput').value);";
  html += "if (isNaN(value)) {";
  html += "alert('Please enter a valid number');";
  html += "return;";
  html += "}";
  html += "if(confirm('Set rest position to: ' + value + '?')) {";
  html += "fetch('/api/set-manual-rest', {";
  html += "method: 'POST',";
  html += "headers: { 'Content-Type': 'application/json' },";
  html += "body: JSON.stringify({value: value})";
  html += "})";
  html += ".then(response => response.text())";
  html += ".then(data => {";
  html += "alert('Rest position set! ' + data);";
  html += "document.getElementById('manualRestInput').value = '';";
  html += "updateData();";
  html += "});";
  html += "}";
  html += "}";

  html += "function setManualForward() {";
  html += "const value = parseFloat(document.getElementById('manualForwardInput').value);";
  html += "if (isNaN(value)) {";
  html += "alert('Please enter a valid number');";
  html += "return;";
  html += "}";
  html += "if(confirm('Set forward position to: ' + value + '?')) {";
  html += "fetch('/api/set-manual-forward', {";
  html += "method: 'POST',";
  html += "headers: { 'Content-Type': 'application/json' },";
  html += "body: JSON.stringify({value: value})";
  html += "})";
  html += ".then(response => response.text())";
  html += ".then(data => {";
  html += "alert('Forward position set! ' + data);";
  html += "document.getElementById('manualForwardInput').value = '';";
  html += "updateData();";
  html += "});";
  html += "}";
  html += "}";
...
void handleSetForward() {
  if (restPosition == 0) {
    server.send(400, "text/plain", "Please calibrate rest position first");
    return;
  }
  
  forwardPosition = selectedAxisValue;
  isCalibrated = true;
  saveConfiguration();
  
  String response = "Forward set to: " + String(forwardPosition) + " (Range: " + String(abs(forwardPosition - restPosition)) + ")";
  Serial.print("Manual forward position set: "); Serial.println(forwardPosition);
  Serial.print("Movement range: "); Serial.println(abs(forwardPosition - restPosition));
  
  server.send(200, "text/plain", response);
}

void handleSetManualRest() {
  if (server.hasArg("plain")) {
    ...
    String response = "Rest position set to: " + String(restPosition);
    Serial.print("Manual rest position set: "); Serial.println(restPosition);
...
void handleSetManualForward() {
  if (server.hasArg("plain")) {
    ...
    String response = "Forward position set to: " + String(forwardPosition) + " (Range: " + String(abs(forwardPosition - restPosition)) + ")";
    Serial.print("Manual forward position set: "); Serial.println(forwardPosition);
    Serial.print("Movement range: "); Serial.println(abs(forwardPosition - restPosition));
