...
void detectStepPattern() {
  // Check if magnitude is sufficient
  if (deltaMagnitude < magnitudeThreshold) {
    return;
  }
  
  // Determine current movement state based on calibrated values
  MovementState newState = STATE_REST;
  
  // Check if we're in forward or backward position
  if (abs(selectedAxisValue - forwardMax) < abs(selectedAxisValue - restPosition) && 
      abs(selectedAxisValue - forwardMax) < abs(selectedAxisValue - backwardMax)) {
    newState = STATE_FORWARD;
  } else if (abs(selectedAxisValue - backwardMax) < abs(selectedAxisValue - restPosition) && 
             abs(selectedAxisValue - backwardMax) < abs(selectedAxisValue - forwardMax)) {
...
void handleRoot() {
  ...
  html += "<canvas id='arcCanvas' width='300' height='200'></canvas>";
  html += "</div>";
  html += "<div style='margin-top: 10px; font-size: 12px; color: #666;'>";
  html += "Yellow: Rest | Blue: Forward | Red: Backward | Gradient Line: Current Movement";
  html += "</div>";
  html += "</div>";
  ...
  html += "const centerX = 150, centerY = 180, radius = 120;";
  
  html += "ctx.clearRect(0, 0, 300, 200);";
  
  html += "ctx.strokeStyle = '#ddd';";
  html += "ctx.lineWidth = 4;";
  html += "ctx.beginPath();";
  html += "ctx.arc(centerX, centerY, radius, Math.PI, 0);";
  html += "ctx.stroke();";
  
  html += "if (restPos !== 0 && forwardPos !== 0 && backwardPos !== 0) {";
  html += "let range = Math.abs(forwardPos - backwardPos);";
  html += "let currentValue = parseFloat(document.getElementById('selectedAxisValue').textContent) || restPos;";
  
  html += "function drawMarker(value, color, label) {";
  html += "let normalized = (value - backwardPos) / range;";
  html += "let angle = Math.PI * normalized;";
  ...
  html += "ctx.font = 'bold 12px Arial';";
  html += "ctx.textAlign = 'center';";
  html += "ctx.fillText(label, x, y - 15);";
  html += "}";
  
  html += "drawMarker(restPos, '#FFC107', 'Rest');";
  html += "drawMarker(forwardPos, '#2196F3', 'Forward');";
  html += "drawMarker(backwardPos, '#f44336', 'Backward');";
  
  html += "let currentNormalized = (currentValue - backwardPos) / range;";
  html += "let restNormalized = (restPos - backwardPos) / range;";
  html += "let startAngle = Math.PI * restNormalized;";
  html += "let endAngle = Math.PI * currentNormalized;";
  
  html += "ctx.lineWidth = 8;";
  html += "let gradient = ctx.createLinearGradient(";
  html += "centerX - radius * Math.cos(startAngle), centerY - radius * Math.sin(startAngle),";
  html += "centerX - radius * Math.cos(endAngle), centerY - radius * Math.sin(endAngle));";
  
  html += "if (currentValue > restPos) {";
  html += "gradient.addColorStop(0, '#FFC107');";
  html += "gradient.addColorStop(1, '#2196F3');";
  html += "} else if (currentValue < restPos) {";
  html += "gradient.addColorStop(0, '#FFC107');";
  html += "gradient.addColorStop(1, '#f44336');";
  html += "} else {";
  html += "gradient.addColorStop(0, '#FFC107');";
  html += "gradient.addColorStop(1, '#FFC107');";
  html += "}";
  
  html += "ctx.strokeStyle = gradient;";
  html += "ctx.beginPath();";
  html += "ctx.arc(centerX, centerY, radius, Math.min(startAngle, endAngle), Math.max(startAngle, endAngle));";
  html += "ctx.stroke();";
  html += "}";
  html += "}";
