...
// mDNS Configuration
const char* mdnsHostname = "garba";
unsigned long lastMDNSUpdate = 0;
const unsigned long mdnsUpdateInterval = 30000; // Refresh mDNS every 30 seconds
...
// New mDNS and WiFi management functions
void initializeMDNS();
void updateMDNS();
...
void initializeMDNS() {
  ...
  // End any existing mDNS instance
  MDNS.end();
  delay(100);
  
  // Start mDNS with error handling
  if (MDNS.begin(mdnsHostname)) {
    Serial.println("✅ mDNS started successfully as: " + String(mdnsHostname) + ".local");
    
    // Add HTTP service
    MDNS.addService("http", "tcp", 80);
    
    // Add custom service for discovery
    MDNS.addService("garba", "tcp", 80);
    
    // Add service properties
    MDNS.addServiceTxt("http", "tcp", "version", currentRunningVersion);
    MDNS.addServiceTxt("http", "tcp", "device", "garba-server");
    MDNS.addServiceTxt("garba", "tcp", "type", "step-counter-server");
    
    mdnsStarted = true;
    lastMDNSUpdate = millis();
    
    Serial.println("🔍 mDNS services registered:");
    Serial.println("   - HTTP service on port 80");
    Serial.println("   - Garba service on port 80");
...
void updateMDNS() {
  // Only update if mDNS was started and enough time has passed
  if (!mdnsStarted || (millis() - lastMDNSUpdate < mdnsUpdateInterval)) {
    return;
  }
  
  // Check if WiFi is still connected
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("⚠️  WiFi disconnected - mDNS update skipped");
    mdnsStarted = false;
    return;
  }
  
  // For ESP32 Core 3.3.0, we don't call MDNS.update() as it doesn't exist
  // Instead, we just mark the time and rely on periodic restart
  lastMDNSUpdate = millis();
  
  // Periodically restart mDNS to prevent issues (every 5 minutes)
  static unsigned long lastMDNSRestart = 0;
  if (millis() - lastMDNSRestart > 300000) { // 5 minutes
    Serial.println("🔄 Refreshing mDNS registration...");
    initializeMDNS();
...
void checkWiFiConnection() {
  ...
  if (status == WL_CONNECTED) {
    if (!wifiConnected) {
      ...
      // Reinitialize mDNS after WiFi reconnection
...
void reconnectWiFi() {
  ...
  // Try WiFiMulti reconnection first
  if (wifiMulti.run() == WL_CONNECTED) {
    Serial.println("✅ WiFi reconnected via WiFiMulti");
    wifiConnected = true;
    return;
  }
  
  // If WiFiMulti fails, try manual reconnection
  WiFi.disconnect();
  delay(100);
...
void checkForOTAUpdate() {
  ...
  if (connectToWiFiMulti()) {
    ...
    wifiConnected = true; // Set WiFi status
...
// handleMainDashboard(), handleDebugConsole(), handleDashboardData(), 
// handleDebugData(), handleReset(), resetAllData()
